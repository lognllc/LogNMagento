...
- (id)init
{
    if (self = [super init]) {
        ...
        [client.requestSerializer setValue:@"urn:Mage_Api_Model_Server_HandlerAction" forHTTPHeaderField:@"SOAPAction"];
...
- (void)createCart:(void (^)(NSNumber *cart))block
{
    ...
    if (!cartID) {
        [self call:@[@"cart.create", storeID] success:^(id responseObject) {
            
            NSLog(@"create cart %@", responseObject);
            NSNumber *_cardID = responseObject;
            [self call:@[@"cart_customer.set", responseObject, @{@"entity_id": customerID, @"mode": @"customer"}, storeID] success:^(id responseObject1) {
                self.cartID = _cardID;
...
+ (void)getImageAndPrice:(NSMutableDictionary *)item completion:(void (^)(BOOL immediate))completion
{
    ...
    if (!url || (!price && !basePrice)) {
        ...
        [self multiCall:calls success:^(id responseObject) {
...
{
    [self call:@[@"catalog_product_attribute_media.list", productId] success:^(NSArray *products) {
...
- (void)startSession
{
    ...
    [client postPath:@"login" parameters:@{@"username": MAGENTO_USERNAME, @"apiKey": MAGENTO_API_KEY} success:^(id responseObject) {
...
+ (void)call:(NSArray *)args success:(MagentoRequestSuccess)success
     failure:(MagentoRequestError)failure
{
    [self.service call:args success:success failure:failure];
}

- (void)call:(NSArray *)params success:(MagentoRequestSuccess)success
     failure:(MagentoRequestError)failure
{
...
- (void)multiCall:(NSArray *)args success:(MagentoRequestSuccess)success
          failure:(MagentoRequestError)failure
{
    [self inSession:^(NSString *session) {
